...
def parse_args():
    parser = argparse.ArgumentParser(description='Depth Anything V2 Distributed Inference')
...
def main():
    ...
    img_list = []
    for index, row in tqdm(df.iterrows(), total=len(df), desc="Loading images", disable=(local_rank != 0)):
    ...
    load_from = os.path.join(args.checkpoints_path, f'Depth-Anything/depth_anything_v2_{args.encoder}.pth')
    depth_anything.load_state_dict(torch.load(load_from, map_location='cpu'))
    depth_anything = depth_anything.to(DEVICE).eval()
    
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Depth inference", disable=(local_rank != 0)):
            ...
            depth = F.interpolate(depth[:,None], size=(original_h, original_w), mode='bilinear', align_corners=False)
            
            for i in range(depth.shape[0]):
                depth_i = depth[i, 0].cpu().numpy()
                img_path = paths[i]
                output_filename = os.path.splitext(os.path.basename(img_path))[0] + '.npy'
                output_dir = os.path.join(os.path.dirname(os.path.dirname(img_path)), 'depth-anything')
