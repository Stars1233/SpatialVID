...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        def inner_func(idx):
            ...
            rgb_torch = torch.from_numpy(rgb_resized).permute(2, 0, 1).float()  # CHW格式
...
def main():
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Processing batches", disable=(local_rank != 0)):
            ...
            for i in range(len(paths)):
                ...
                focal_length = intrinsics[0, 0]  # 假设主点在中心，取fx
                w = depth.shape[-1] # 宽度
                ...
                # 保存结果
                img_path = paths[i]
                output_filename = os.path.splitext(os.path.basename(img_path))[0] + '.npz'
                output_dir = os.path.join(os.path.dirname(os.path.dirname(img_path)), 'unidepth')
