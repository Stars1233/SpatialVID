...
def process_single_row(row, detect_id, frame_skip=0, adaptive_threshold=3, content_threshold=27,
                       start_remove_sec=0, end_remove_sec=0, min_seconds=2, max_seconds=15):
    video_path = row["video_path"]
    if detect_id == 0:
        detector = ContentDetector(
            threshold=10,  # default=27
        )  # can detect fade in/out
    elif detect_id == 1:
        detector = AdaptiveDetector(
            adaptive_threshold=3,  # default=3
            # luma_only=True,
        )
    elif detect_id == 2:
        detector = ContentDetector(
            threshold=21,  # default=27
        )  # can detect fade in/out

    elif detect_id == 3:
        detector = ContentDetector(
            threshold=15,  # default=27
        )  # can detect fade in/out

    elif detect_id == 4:
        detector = ContentDetector(
            threshold=27,  # default=27
        )  # can detect fade in/out

    elif detect_id == 5:
        detector = AdaptiveDetector(
            adaptive_threshold=1,  # default=3
            # luma_only=True,
        )
    elif detect_id == 6:
        detector = AdaptiveDetector(
    ...
    elif detect_id == 8:  # patch_test2
        detector1 = ContentDetector(threshold=18, min_scene_len=15)
        detector2 = AdaptiveDetector(adaptive_threshold=3.0, min_scene_len=15)
        detector = [detector1, detector2]
    elif detect_id == 8:  # sophia
        detector = AdaptiveDetector(
    ...
    try:
        ...
        # 处理时间戳，删除起始和结尾指定秒数，过滤时长小于指定值的时间戳
        new_timestamp = []
        ...
        for start_timecode, end_timecode in timestamp:
            ...
            if duration >= total_remove_sec:
                ...
                if new_duration <= max_seconds:
                    # 长度小于等于max_seconds，直接判断是否满足min_seconds
...
def parse_args():
    ...
    parser.add_argument("--detect_id", type=int, default=None, help="Detector ID")
    parser.add_argument("--frame_skip", type=int, default=None, help="skip frame for detect_scenes")
    parser.add_argument("--adaptive_threshold", type=float, default=3, help="Adaptive threshold for AdaptiveDetector")
    parser.add_argument("--content_threshold", type=float, default=27, help="Content threshold for AdaptiveDetector")
