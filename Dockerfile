...
RUN set -eux; \
        ...
        if ! git clone --depth 1 --branch "${NV_CODEC_HEADERS_BRANCH}" https://github.com/FFmpeg/nv-codec-headers.git /workspace/build/nv-codec-headers; then \
            git clone https://github.com/FFmpeg/nv-codec-headers.git /workspace/build/nv-codec-headers; \
...
RUN set -eux; \
    git clone https://github.com/Netflix/vmaf.git /workspace/build/vmaf; \
...
RUN set -eux; \
        ...
        git clone https://github.com/FFmpeg/FFmpeg.git /workspace/build/FFmpeg; \
